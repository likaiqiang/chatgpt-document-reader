...
export const getTreeSitterBinding = (paths = [])=>{
    const bp =  paths.reduce(
        ...
        MAIN_WINDOW_VITE_DEV_SERVER_URL ? path.join(process.cwd(),'src','assets','node') : path.join(__dirname,'node')
    )
    return require(bp)
}
...
export const {Query, Parser, NodeMethods, Tree, TreeCursor} = binding;
