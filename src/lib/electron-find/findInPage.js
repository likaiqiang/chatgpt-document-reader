...
import { print, on, off, move } from './utils.js'
...
function onInput () {
  setTimeout(async () => {
    ...
    if (text && text !== this[lastText]) {
      ...
      await this.startFind(text, true, this[matchCase])
...
async function onCaseClick () {
  if (!this[matchCase]) {
    this[matchCase] = true
    this[findCase].style['border-color'] = this[config].caseSelectedColor
    wrapInput(this[findInput], this[findCase], 100)
    await this.startFind(this[findInput].value, true, this[matchCase])
  } else {
    this[matchCase] = false
...
async function onBackClick () {
  this[action] = 'back'
  wrapInput(this[findInput], this[findCase], 100)
  await this.findNext(false, this[matchCase])
}

async function onForwardClick () {
  this[action] = 'forward'
  wrapInput(this[findInput], this[findCase], 100)
  await this.findNext(true, this[matchCase])
}
