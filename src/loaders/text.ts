...
class TextLoader {
    async parse(path:string): Promise<Document<Record<string, any>>[]>{
        const now = Date.now()
        const jsonPath = MAIN_WINDOW_VITE_DEV_SERVER_URL ? filepath.join(process.cwd(),'src','assets','python_code','result',`semantic_splitter_text_${now}.json`) : filepath.join(__dirname,'python_code','result',`semantic_splitter_text_${now}.json`)
